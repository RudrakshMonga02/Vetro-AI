"""
Repository interface for case data access. This is the seam that lets
the same FastAPI routes/services work against either:
  - Postgres/Supabase (via SQLAlchemy)   -> db/repository_postgres.py
  - Catalyst Data Store (via ZCQL/SDK)   -> catalyst/repository_catalyst.py

Routes and services should only ever depend on this interface, never
on SQLAlchemy or zcatalyst_sdk directly. Swapping backends means
swapping which implementation gets constructed (see repository_factory.py),
